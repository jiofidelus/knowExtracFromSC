...
import org.imogene.epicam.client.ui.editor.PriseMedicamentRegimeEditor;
...
/**
 * Worflow that manages the life of a PriseMedicamentRegimeProxy in the UI
 * @author MEDES-IMPS
 */
public class PriseMedicamentRegimeEditorWorkflow
		extends
			EditorWorkflowComposite {

	interface Driver
			extends
				RequestFactoryEditorDriver<PriseMedicamentRegimeProxy, PriseMedicamentRegimeEditor> {
	}

	private EpicamRequestFactory requestFactory;

	private PriseMedicamentRegimeRequest request;
	public PriseMedicamentRegimeProxy current;
	private Driver editorDriver;
	private PriseMedicamentRegimeEditor editor;
	private String initField;
	private boolean showGlassPanel = false;

	/**
	 * Workflow constructor for the creation of a PriseMedicamentRegime instance
	 * @param factory the application request factory
	 * @param titleContainer the Label that will display the workflow title
	 */
	public PriseMedicamentRegimeEditorWorkflow(EpicamRequestFactory factory,
	...
	public PriseMedicamentRegimeEditorWorkflow(EpicamRequestFactory factory,
		...
		if (parent != null) {
			editor = new PriseMedicamentRegimeEditor(factory, true);
			this.initField = initField;
		} else
			editor = new PriseMedicamentRegimeEditor(factory);

		isNew = true;
		setEditable(true);

		setTitle(NLS.constants().priseMedicamentRegime_create_title());
		createDriver();
		createNewPriseMedicamentRegime();

		this.setContent(editor);
	}

	/**
	 * Workflow constructor for the visualization and edition of an existing PriseMedicamentRegime instance
	 * @param factory the application request factory
	 * @param entityId the id of the PriseMedicamentRegime instance to be visualized and edited	 
	 * @param titleContainer the Label that will display the workflow title	 
	 */
	public PriseMedicamentRegimeEditorWorkflow(EpicamRequestFactory factory,
	...
	public PriseMedicamentRegimeEditorWorkflow(EpicamRequestFactory factory,
		...
		createDriver();
		fetchPriseMedicamentRegime(entityId);
	...
	/**
	 * Create a new instance of PriseMedicamentRegime
	 */
	private void createNewPriseMedicamentRegime() {

		request = requestFactory.priseMedicamentRegimeRequest();

		/* create a new intance of PriseMedicamentRegime */
		...
		current = newPriseMedicamentRegime;
	...
	/**
	 * Get an existing instance of PriseMedicamentRegime
	 * @param entityId the id of the PriseMedicamentRegimeProxy to be fetched
	 */
	private void fetchPriseMedicamentRegime(String entityId) {

		PriseMedicamentRegimeRequest request = requestFactory
				.priseMedicamentRegimeRequest();

		/* get the PriseMedicamentRegime instance from database */
		Request<PriseMedicamentRegimeProxy> fetchRequest = request
				.findById(entityId);
		fetchRequest.with("regime");
		fetchRequest.with("medicament");

		fetchRequest.to(new Receiver<PriseMedicamentRegimeProxy>() {
			@Override
			public void onSuccess(PriseMedicamentRegimeProxy entity) {
				viewPriseMedicamentRegime(entity);
			}
		}).fire();
	}

	/**
	 * Display the current instance of PriseMedicamentRegime in editor
	 * @param entity the PriseMedicamentRegimeProxy to be displayed
	 */
	private void viewPriseMedicamentRegime(PriseMedicamentRegimeProxy entity) {

		/* display instance information */
		setTitle(NLS.constants().priseMedicamentRegime_name() + ": "
	...
	/**
	 * Edit the current instance of PriseMedicamentRegime in editor
	 */
	@Override
	protected void edit() {
	...
	/**
	 * Persist the current instance of PriseMedicamentRegime
	 */
	@Override
	protected void save() {
		...
		if (editorDriver.hasErrors()) {
			//Window.alert("PriseMedicamentRegime form not validated locally");
			return;
		}
		...
		saveRequest.to(new Receiver<Void>() {
			...
			public void onSuccess(Void response) {
				requestFactory.getEventBus().fireEvent(
						new SavePriseMedicamentRegimeEvent(current, initField));
				closeForm();
			}

			@Override
			public void onConstraintViolation(Set<ConstraintViolation<?>> errors) {
				//Window.alert("PriseMedicamentRegime form not validated on server");
			...
			public void onFailure(ServerFailure error) {
				Window.alert("Error updating the PriseMedicamentRegime");
	...
	protected void cancel() {
		if (parent != null)
			...
			if (isNew)
				...
				requestFactory.getEventBus().fireEvent(
						new ViewPriseMedicamentRegimeEvent(current.getId(),
								closeEvent));
		}
	...
	public void setRegime(RegimeProxy value, boolean isLocked) {
		editor.setRegime(value, isLocked);
	}
	/**
