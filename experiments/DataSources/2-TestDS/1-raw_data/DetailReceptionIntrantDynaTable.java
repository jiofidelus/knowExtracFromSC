...
import org.imogene.epicam.client.event.create.CreateDetailReceptionIntrantEvent;
...
/**
 * Composite that displays the list of DetailReceptionIntrant entries
 * @author MEDES-IMPS
 */
public class DetailReceptionIntrantDynaTable
		extends
			ImogDynaTable<DetailReceptionIntrantProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public DetailReceptionIntrantDynaTable(EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<DetailReceptionIntrantProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new DetailReceptionIntrantFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadDetailReceptionIntrantDescription()) {
			Column<DetailReceptionIntrantProxy, String> commandeColumn = new CommandeColumn();
			commandeColumn.setSortable(true);
			table.addColumn(commandeColumn, NLS.constants()
					.detailReceptionIntrant_field_s_commande());
		}
		if (AccessManager.canReadDetailReceptionIntrantDescription()) {
			Column<DetailReceptionIntrantProxy, String> detailCommandeColumn = new DetailCommandeColumn();
			detailCommandeColumn.setSortable(true);
			table.addColumn(detailCommandeColumn, NLS.constants()
					.detailReceptionIntrant_field_s_detailCommande());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(DetailReceptionIntrantProxy value) {
		History.newItem(TokenHelper.TK_VIEW + "/detailreceptionintrant/"
				+ value.getId(), true);
		return null;
	}
		...
		if (AccessManager.canDeleteDetailReceptionIntrant()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<DetailReceptionIntrantProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						msg.append(BaseNLS.constants()
								...
								+ " "
								+ NLS.constants().detailReceptionIntrant_name()
								+ " "
								+ BaseNLS.constants()
										.confirmation_delete_several2() + ": ");
						int i = 0;
						for (DetailReceptionIntrantProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getDetailReceptionIntrantRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected DetailReceptionIntrant entries have been deleted");
									requestFactory
											.getEventBus()
											.fireEvent(
								...
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the DetailReceptionIntrant entries");
	...
	/**
	 * Creates the action command that enables to export the DetailReceptionIntrant
	 * entries in a csv file
	 * @return the command
	 */
	public Command getCsvExportButton() {

		if (AccessManager.canExportDetailReceptionIntrant()) {

			Command command = new Command() {
				public void execute() {

					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.DET_REC_INT_CSV_KEY + "?"
							+ EpicamBirtConstants.REPORT_NAME
							+ "=detailReceptionIntrant_csv" + "&"
	...
	private DetailReceptionIntrantRequest getDetailReceptionIntrantRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.detailReceptionIntrantRequest();
	}
	...
	/**
	 * Column for field Commande
	 * @author MEDES-IMPS
	 */
	private class CommandeColumn
			extends
				ImogColumn<DetailReceptionIntrantProxy, String> {

		private EpicamRenderer renderer = EpicamRenderer.get();

		public CommandeColumn() {
			super(new TextCell());
		}

		@Override
		public String getValue(DetailReceptionIntrantProxy object) {
			String value = null;
			if (object != null) {
				if (object.getCommande() == null)
					value = "";
				else
					value = renderer.getDisplayValue(object.getCommande());
			}
			return value;
		}
	...
	/**
	 * Column for field DetailCommande
	 * @author MEDES-IMPS
	 */
	private class DetailCommandeColumn
			extends
				ImogColumn<DetailReceptionIntrantProxy, String> {

		private EpicamRenderer renderer = EpicamRenderer.get();

		public DetailCommandeColumn() {
			super(new TextCell());
		}
		...
		public String getValue(DetailReceptionIntrantProxy object) {
			String value = null;
			if (object != null) {
				if (object.getDetailCommande() == null)
					value = "";
				else
					value = renderer
							.getDisplayValue(object.getDetailCommande());
			}
			return value;
		}
