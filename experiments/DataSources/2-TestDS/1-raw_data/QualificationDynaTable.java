...
import org.imogene.epicam.client.event.create.CreateQualificationEvent;
...
/**
 * Composite that displays the list of Qualification entries
 * @author MEDES-IMPS
 */
public class QualificationDynaTable extends ImogDynaTable<QualificationProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public QualificationDynaTable(EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<QualificationProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new QualificationFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadQualificationDescription()) {
			Column<QualificationProxy, String> codeColumn = new CodeColumn();
			codeColumn.setSortable(true);
			table.addColumn(codeColumn, NLS.constants()
					.qualification_field_s_code());
		}
		if (AccessManager.canReadQualificationDescription()) {
			Column<QualificationProxy, String> nomColumn = new NomColumn();
			nomColumn.setSortable(true);
			table.addColumn(nomColumn, NLS.constants()
					.qualification_field_s_nom());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(QualificationProxy value) {
		History.newItem(
				TokenHelper.TK_VIEW + "/qualification/" + value.getId(), true);
		return null;
	}
		...
		if (AccessManager.canDeleteQualification()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<QualificationProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						msg.append(BaseNLS.constants()
								...
								+ " "
								+ NLS.constants().qualification_name()
								+ " "
								+ BaseNLS.constants()
										.confirmation_delete_several2() + ": ");
						int i = 0;
						for (QualificationProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getQualificationRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected Qualification entries have been deleted");
									requestFactory.getEventBus().fireEvent(
											new ListQualificationEvent());
								}

								@Override
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the Qualification entries");
	...
	/**
	 * Creates the action command that enables to export the Qualification
	 * entries in a csv file
	 * @return the command
	 */
	public Command getCsvExportButton() {

		if (AccessManager.canExportQualification()) {

			Command command = new Command() {
				public void execute() {

					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.QUA_CSV_KEY + "?"
	...
	private QualificationRequest getQualificationRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.qualificationRequest();
	}
	 ...
	 * @author MEDES-IMPS
	 */
	private class CodeColumn extends ImogColumn<QualificationProxy, String> {
		...
		@Override
		public String getValue(QualificationProxy object) {
			String value = null;
			if (object != null) {
	 ...
	 * @author MEDES-IMPS
	 */
	private class NomColumn extends ImogColumn<QualificationProxy, String> {
