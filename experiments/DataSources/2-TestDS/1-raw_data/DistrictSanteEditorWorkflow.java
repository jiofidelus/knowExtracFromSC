...
import org.imogene.epicam.client.ui.editor.DistrictSanteEditor;
...
/**
 * Worflow that manages the life of a DistrictSanteProxy in the UI
 * @author MEDES-IMPS
 */
public class DistrictSanteEditorWorkflow extends EditorWorkflowComposite {

	interface Driver
			extends
				RequestFactoryEditorDriver<DistrictSanteProxy, DistrictSanteEditor> {
	}

	private EpicamRequestFactory requestFactory;

	private DistrictSanteRequest request;
	public DistrictSanteProxy current;
	private Driver editorDriver;
	private DistrictSanteEditor editor;
	private String initField;
	private boolean showGlassPanel = false;

	/**
	 * Workflow constructor for the creation of a DistrictSante instance
	 * @param factory the application request factory
	 * @param titleContainer the Label that will display the workflow title
	 */
	public DistrictSanteEditorWorkflow(EpicamRequestFactory factory,
	...
	public DistrictSanteEditorWorkflow(EpicamRequestFactory factory,
		...
		if (parent != null) {
			editor = new DistrictSanteEditor(factory, true);
			this.initField = initField;
		} else
			editor = new DistrictSanteEditor(factory);

		isNew = true;
		setEditable(true);

		setTitle(NLS.constants().districtSante_create_title());
		createDriver();
		createNewDistrictSante();

		this.setContent(editor);
	}

	/**
	 * Workflow constructor for the visualization and edition of an existing DistrictSante instance
	 * @param factory the application request factory
	 * @param entityId the id of the DistrictSante instance to be visualized and edited	 
	 * @param titleContainer the Label that will display the workflow title	 
	 */
	public DistrictSanteEditorWorkflow(EpicamRequestFactory factory,
	...
	/**
	 * Create a new instance of DistrictSante
	 */
	private void createNewDistrictSante() {

		request = requestFactory.districtSanteRequest();

		/* create a new intance of DistrictSante */
		DistrictSanteProxy newDistrictSante = request
				.create(DistrictSanteProxy.class);
		newDistrictSante.setId(ImogKeyGenerator.generateKeyId("DST"));
		LocalizedTextProxy newNom = request.create(LocalizedTextProxy.class);
		newDistrictSante.setNom(newNom);
		LocalizedTextProxy newDescription = request
				.create(LocalizedTextProxy.class);
		newDistrictSante.setDescription(newDescription);
		GeoFieldProxy newCoordonnees = request.create(GeoFieldProxy.class);
		newDistrictSante.setCoordonnees(newCoordonnees);

		/* push the instance to the editor */
		current = newDistrictSante;
	...
	/**
	 * Get an existing instance of DistrictSante
	 * @param entityId the id of the DistrictSanteProxy to be fetched
	 */
	private void fetchDistrictSante(String entityId) {

		DistrictSanteRequest request = requestFactory.districtSanteRequest();

		/* get the DistrictSante instance from database */
		Request<DistrictSanteProxy> fetchRequest = request.findById(entityId);
		...
		fetchRequest.to(new Receiver<DistrictSanteProxy>() {
			@Override
			public void onSuccess(DistrictSanteProxy entity) {
				viewDistrictSante(entity);
			}
		}).fire();
	}

	/**
	 * Display the current instance of DistrictSante in editor
	 * @param entity the DistrictSanteProxy to be displayed
	 */
	private void viewDistrictSante(DistrictSanteProxy entity) {

		/* display instance information */
		setTitle(NLS.constants().districtSante_name() + ": "
	...
	/**
	 * Edit the current instance of DistrictSante in editor
	 */
	@Override
	protected void edit() {
	...
	/**
	 * Persist the current instance of DistrictSante
	 */
	@Override
	protected void save() {
		...
		if (editorDriver.hasErrors()) {
			//Window.alert("DistrictSante form not validated locally");
			return;
		}
		...
		saveRequest.to(new Receiver<Void>() {
			...
			public void onSuccess(Void response) {
				requestFactory.getEventBus().fireEvent(
						new SaveDistrictSanteEvent(current, initField));
				closeForm();
			}

			@Override
			public void onConstraintViolation(Set<ConstraintViolation<?>> errors) {
				//Window.alert("DistrictSante form not validated on server");
			...
			public void onFailure(ServerFailure error) {
				Window.alert("Error updating the DistrictSante");
		...
		if (parent != null)
			...
			if (isNew)
				...
				requestFactory.getEventBus()
						.fireEvent(
								new ViewDistrictSanteEvent(current.getId(),
										closeEvent));
		}
	...
	protected void returnToList() {
		requestFactory.getEventBus().fireEvent(new ListDistrictSanteEvent());
	}
