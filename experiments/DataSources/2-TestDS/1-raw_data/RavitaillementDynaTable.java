...
import org.imogene.epicam.client.event.create.CreateRavitaillementEvent;
...
/**
 * Composite that displays the list of Ravitaillement entries
 * @author MEDES-IMPS
 */
public class RavitaillementDynaTable extends ImogDynaTable<RavitaillementProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public RavitaillementDynaTable(EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<RavitaillementProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new RavitaillementFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadRavitaillementInformationsDepart()) {
			Column<RavitaillementProxy, String> cDTDepartColumn = new CDTDepartColumn();
			cDTDepartColumn.setSortable(true);
			table.addColumn(cDTDepartColumn, NLS.constants()
					.ravitaillement_field_s_cDTDepart());
		}
		if (AccessManager.canReadRavitaillementInformationsDepart()) {
			Column<RavitaillementProxy, String> dateDepartColumn = new DateDepartColumn();
			dateDepartColumn.setSortable(true);
			table.addColumn(dateDepartColumn, NLS.constants()
					.ravitaillement_field_s_dateDepart());
		}
		if (AccessManager.canReadRavitaillementInformationArrivee()) {
			Column<RavitaillementProxy, String> cDTArriveeColumn = new CDTArriveeColumn();
			cDTArriveeColumn.setSortable(true);
			table.addColumn(cDTArriveeColumn, NLS.constants()
					.ravitaillement_field_s_cDTArrivee());
		}
		if (AccessManager.canReadRavitaillementInformationArrivee()) {
			Column<RavitaillementProxy, String> dateArriveeColumn = new DateArriveeColumn();
			dateArriveeColumn.setSortable(true);
			table.addColumn(dateArriveeColumn, NLS.constants()
					.ravitaillement_field_s_dateArrivee());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(RavitaillementProxy value) {
		History.newItem(
				TokenHelper.TK_VIEW + "/ravitaillement/" + value.getId(), true);
		return null;
	}
		...
		if (AccessManager.canDeleteRavitaillement()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<RavitaillementProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						msg.append(BaseNLS.constants()
								...
								+ " "
								+ NLS.constants().ravitaillement_name()
								+ " "
								+ BaseNLS.constants()
										.confirmation_delete_several2() + ": ");
						int i = 0;
						for (RavitaillementProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getRavitaillementRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected Ravitaillement entries have been deleted");
									requestFactory.getEventBus().fireEvent(
											new ListRavitaillementEvent());
								}

								@Override
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the Ravitaillement entries");
	...
	/**
	 * Creates the action command that enables to export the Ravitaillement
	 * entries in a csv file
	 * @return the command
	 */
	public Command getCsvExportButton() {

		if (AccessManager.canExportRavitaillement()) {

			Command command = new Command() {
				public void execute() {

					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.RAV_CSV_KEY + "?"
	...
	private RavitaillementRequest getRavitaillementRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.ravitaillementRequest();
	}
	...
	private class CDTDepartColumn
			extends
				ImogColumn<RavitaillementProxy, String> {
		...
		@Override
		public String getValue(RavitaillementProxy object) {
			String value = null;
			if (object != null) {
