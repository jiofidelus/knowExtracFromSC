...
import org.imogene.epicam.client.event.create.CreateDetailReceptionMedicamentEvent;
...
/**
 * Composite that displays the list of DetailReceptionMedicament entries
 * @author MEDES-IMPS
 */
public class DetailReceptionMedicamentDynaTable
		extends
			ImogDynaTable<DetailReceptionMedicamentProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public DetailReceptionMedicamentDynaTable(
			EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<DetailReceptionMedicamentProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new DetailReceptionMedicamentFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadDetailReceptionMedicamentDescription()) {
			Column<DetailReceptionMedicamentProxy, String> commandeColumn = new CommandeColumn();
			commandeColumn.setSortable(true);
			table.addColumn(commandeColumn, NLS.constants()
					.detailReceptionMedicament_field_s_commande());
		}
		if (AccessManager.canReadDetailReceptionMedicamentDescription()) {
			Column<DetailReceptionMedicamentProxy, String> detailCommandeColumn = new DetailCommandeColumn();
			detailCommandeColumn.setSortable(true);
			table.addColumn(detailCommandeColumn, NLS.constants()
					.detailReceptionMedicament_field_s_detailCommande());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(DetailReceptionMedicamentProxy value) {
		History.newItem(TokenHelper.TK_VIEW + "/detailreceptionmedicament/"
				+ value.getId(), true);
		return null;
	}
			...
			Command command = new Command() {
				public void execute() {
					History.newItem(TokenHelper.TK_NEW
							+ "/detailreceptionmedicament/", true);
				}
			};
			return command;
		} else
		...
		if (AccessManager.canDeleteDetailReceptionMedicament()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<DetailReceptionMedicamentProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						int i = 0;
						for (DetailReceptionMedicamentProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getDetailReceptionMedicamentRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected DetailReceptionMedicament entries have been deleted");
									requestFactory
											.getEventBus()
											.fireEvent(
								...
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the DetailReceptionMedicament entries");
	...
	/**
	 * Creates the action command that enables to export the DetailReceptionMedicament
			...
			Command command = new Command() {
				public void execute() {
					...
					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.DET_REC_MED_CSV_KEY + "?"
							+ EpicamBirtConstants.REPORT_NAME
							+ "=detailReceptionMedicament_csv" + "&"
	...
	private DetailReceptionMedicamentRequest getDetailReceptionMedicamentRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.detailReceptionMedicamentRequest();
	}
	...
	private class CommandeColumn
			extends
				ImogColumn<DetailReceptionMedicamentProxy, String> {
		...
		@Override
		public String getValue(DetailReceptionMedicamentProxy object) {
			String value = null;
			if (object != null) {
