...
/**
 * Data provider to feed the Tutoriel entry Table and Selection List
 * @author MEDES-IMPS
 */
public class TutorielDataProvider extends ImogBeanDataProvider<TutorielProxy> {
	...
	/**
	 * For Relation Fields
	 * Provides instances of entity Tutoriel with pagination
	 */
	public TutorielDataProvider(EpicamRequestFactory requestFactory) {
	...
	/**
	 * For Relation Fields
	 * Provides instances of entity Tutoriel that have non affected values for a given property (RelationField with card==1) with pagination
	...
	/**
	 * For Relation Fields
	 * Provides filtered instances of entity Tutoriel that have non affected values for a given property (RelationField with card==1) with pagination
	 * @param pProperty the property for which non affected values are searched
	 ...
	 */
	@Override
	public Request<List<TutorielProxy>> getList(int start, int numRows) {

		TutorielRequest request = (TutorielRequest) getContext();
		Request<List<TutorielProxy>> result = null;
		...
		if (isFiltered) {
			...
			if (filterCriteria != null) {
				...
				if (searchCriterions != null) {
					...
					if (nonAffected) {
						if (!searchInReverse)
							result = request.listNonAffectedTutoriel(start,
									numRows, "nom", true, searchCriterions,
									property);
						else
							result = request.listNonAffectedTutorielReverse(
									start, numRows, "nom", true,
									searchCriterions, property);
					} else
						result = request.listTutoriel(start, numRows, "nom",
				...
				result = request.getTutorielEmptyList();
		} else {
			if (searchCriterions != null) {
				...
				if (nonAffected) {
					if (!searchInReverse)
						...
						result = request.listNonAffectedTutorielReverse(start,
								numRows, "nom", true, searchCriterions,
								property);
				} else
					result = request.listTutoriel(start, numRows, "nom", true,
							searchCriterions);
			} else {
	 ...
	 */
	@Override
	public Request<List<TutorielProxy>> getList(String property, int start,
		...
		if (isFiltered) {
			...
			if (filterCriteria != null) {
				...
				if (searchCriterions != null)
					/* permanent filter added to search criterion */
					result = request.listTutoriel(start, numRows, property,
			...
			if (searchCriterions != null)
				result = request.listTutoriel(start, numRows, property, asc,
						searchCriterions);
			else
				result = request.listTutoriel(start, numRows, property, asc);
		}
		...
		if (isFiltered) {
			...
			if (filterCriteria != null) {
				...
				if (searchCriterions != null) {
					...
					if (nonAffected) {
						if (!searchInReverse)
							return request.countNonAffectedTutoriel(property,
									searchCriterions);
						else
							return request.countNonAffectedTutorielReverse(
									property, searchCriterions);
					} else
						return request.countTutoriel(searchCriterions);
				} else {
					/* permanent filter only */
					if (nonAffected) {
						if (!searchInReverse)
						...
						return request.countTutoriel(filterCriteria);
				}

			} else
				return request.countNonAffectedTutoriel("id");
		} else {
			...
			if (searchCriterions != null) {
				...
				if (nonAffected) {
					if (!searchInReverse)
						...
						return request
								.countNonAffectedTutorielReverse(property);
				} else
					return request.countTutoriel();
			}
		}
	...
	public RequestContext getEntityContext() {
		return requestFactory.tutorielRequest();
	}
	...
	public String fullTextSearch(String text) {
		...
		if (text == null || (text != null && text.equals(""))) {
			...
			buffer.append("(" + NLS.constants().tutoriel_field_nom() + ": "
					+ text + ") " + SYMBOL_OR + " ");
			criterionList.add(nomCrit);

			// Search field Reference
			BasicCriteriaProxy referenceCrit = request
					.create(BasicCriteriaProxy.class);
			referenceCrit.setField("reference");
			referenceCrit
					.setOperation(CriteriaConstants.STRING_OPERATOR_CONTAINS);
			referenceCrit.setValue(text);
			buffer.append("(" + NLS.constants().tutoriel_field_reference()
					+ ": " + text + ") " + SYMBOL_OR + " ");
			criterionList.add(referenceCrit);

			// Search field Type
			if (text.toLowerCase().equals(
					NLS.constants().tutoriel_type_texte_option().toLowerCase())) {
				BasicCriteriaProxy typeCrit = request
						.create(BasicCriteriaProxy.class);
				typeCrit.setField("type");
				typeCrit.setOperation(CriteriaConstants.STRING_OPERATOR_EQUAL);
				typeCrit.setValue(EpicamEnumConstants.TUTORIEL_TYPE_TEXTE);
				buffer.append("(" + NLS.constants().tutoriel_field_type()
						+ ": " + text + ") " + SYMBOL_OR + " ");
				criterionList.add(typeCrit);
			}
			if (text.toLowerCase().equals(
					NLS.constants().tutoriel_type_audio_option().toLowerCase())) {
				BasicCriteriaProxy typeCrit = request
						.create(BasicCriteriaProxy.class);
				typeCrit.setField("type");
				typeCrit.setOperation(CriteriaConstants.STRING_OPERATOR_EQUAL);
				typeCrit.setValue(EpicamEnumConstants.TUTORIEL_TYPE_AUDIO);
				buffer.append("(" + NLS.constants().tutoriel_field_type()
						+ ": " + text + ") " + SYMBOL_OR + " ");
				criterionList.add(typeCrit);
			}
			if (text.toLowerCase().equals(
					NLS.constants().tutoriel_type_video_option().toLowerCase())) {
				BasicCriteriaProxy typeCrit = request
						.create(BasicCriteriaProxy.class);
				typeCrit.setField("type");
				typeCrit.setOperation(CriteriaConstants.STRING_OPERATOR_EQUAL);
				typeCrit.setValue(EpicamEnumConstants.TUTORIEL_TYPE_VIDEO);
				buffer.append("(" + NLS.constants().tutoriel_field_type()
						+ ": " + text + ") " + SYMBOL_OR + " ");
				criterionList.add(typeCrit);
			}
