...
import org.imogene.epicam.client.event.create.CreateReceptionEvent;
...
/**
 * Composite that displays the list of Reception entries
 * @author MEDES-IMPS
 */
public class ReceptionDynaTable extends ImogDynaTable<ReceptionProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public ReceptionDynaTable(EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<ReceptionProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new ReceptionFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadReceptionDescription()) {
			Column<ReceptionProxy, String> cDTColumn = new CDTColumn();
			cDTColumn.setSortable(true);
			table.addColumn(cDTColumn, NLS.constants().reception_field_s_cDT());
		}
		if (AccessManager.canReadReceptionDescription()) {
			Column<ReceptionProxy, String> commandeColumn = new CommandeColumn();
			commandeColumn.setSortable(true);
			table.addColumn(commandeColumn, NLS.constants()
					.reception_field_s_commande());
		}
		if (AccessManager.canReadReceptionDescription()) {
			Column<ReceptionProxy, String> dateReceptionColumn = new DateReceptionColumn();
			dateReceptionColumn.setSortable(true);
			table.addColumn(dateReceptionColumn, NLS.constants()
					.reception_field_s_dateReception());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(ReceptionProxy value) {
		History.newItem(TokenHelper.TK_VIEW + "/reception/" + value.getId(),
				true);
		return null;
	}
		...
		if (AccessManager.canDeleteReception()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<ReceptionProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						msg.append(BaseNLS.constants()
								...
								+ " "
								+ NLS.constants().reception_name()
								+ " "
								+ BaseNLS.constants()
										.confirmation_delete_several2() + ": ");
						int i = 0;
						for (ReceptionProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getReceptionRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected Reception entries have been deleted");
									requestFactory.getEventBus().fireEvent(
											new ListReceptionEvent());
								}

								@Override
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the Reception entries");
	...
	/**
	 * Creates the action command that enables to export the Reception
	 * entries in a csv file
	 * @return the command
	 */
	public Command getCsvExportButton() {

		if (AccessManager.canExportReception()) {

			Command command = new Command() {
				public void execute() {

					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.REC_CSV_KEY + "?"
	...
	private ReceptionRequest getReceptionRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.receptionRequest();
	}
	 ...
	 * @author MEDES-IMPS
	 */
	private class CDTColumn extends ImogColumn<ReceptionProxy, String> {
		...
		@Override
		public String getValue(ReceptionProxy object) {
			String value = null;
			if (object != null) {
	 ...
	 * @author MEDES-IMPS
	 */
	private class CommandeColumn extends ImogColumn<ReceptionProxy, String> {
	...
	/**
	 * Column for field DateReception
	 * @author MEDES-IMPS
	 */
	private class DateReceptionColumn
			extends
				ImogColumn<ReceptionProxy, String> {

		public DateReceptionColumn() {
			super(new TextCell());
		}
		...
		public String getValue(ReceptionProxy object) {
			String value = null;
			if (object != null) {
				if (object.getDateReception() == null)
					value = "";
				else
					value = DateUtil.getFormatedDate(object.getDateReception());
			}
			return value;
		}
