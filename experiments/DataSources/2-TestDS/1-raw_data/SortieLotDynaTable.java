...
import org.imogene.epicam.client.event.create.CreateSortieLotEvent;
...
/**
 * Composite that displays the list of SortieLot entries
 * @author MEDES-IMPS
 */
public class SortieLotDynaTable extends ImogDynaTable<SortieLotProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public SortieLotDynaTable(EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<SortieLotProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new SortieLotFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadSortieLotDescription()) {
			Column<SortieLotProxy, String> lotColumn = new LotColumn();
			lotColumn.setSortable(true);
			table.addColumn(lotColumn, NLS.constants().sortieLot_field_s_lot());
		}
		if (AccessManager.canReadSortieLotDescription()) {
			Column<SortieLotProxy, String> quantiteColumn = new QuantiteColumn();
			quantiteColumn.setSortable(true);
			table.addColumn(quantiteColumn, NLS.constants()
					.sortieLot_field_s_quantite());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(SortieLotProxy value) {
		History.newItem(TokenHelper.TK_VIEW + "/sortielot/" + value.getId(),
				true);
		return null;
	}
		...
		if (AccessManager.canDeleteSortieLot()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<SortieLotProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						msg.append(BaseNLS.constants()
								...
								+ " "
								+ NLS.constants().sortieLot_name()
								+ " "
								+ BaseNLS.constants()
										.confirmation_delete_several2() + ": ");
						int i = 0;
						for (SortieLotProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getSortieLotRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected SortieLot entries have been deleted");
									requestFactory.getEventBus().fireEvent(
											new ListSortieLotEvent());
								}

								@Override
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the SortieLot entries");
	...
	/**
	 * Creates the action command that enables to export the SortieLot
	 * entries in a csv file
	 * @return the command
	 */
	public Command getCsvExportButton() {

		if (AccessManager.canExportSortieLot()) {

			Command command = new Command() {
				public void execute() {

					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.SORT_CSV_KEY + "?"
	...
	private SortieLotRequest getSortieLotRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.sortieLotRequest();
	}
	 ...
	 * @author MEDES-IMPS
	 */
	private class LotColumn extends ImogColumn<SortieLotProxy, String> {
		...
		@Override
		public String getValue(SortieLotProxy object) {
			String value = null;
			if (object != null) {
	 ...
	 * @author MEDES-IMPS
	 */
	private class QuantiteColumn extends ImogColumn<SortieLotProxy, String> {
