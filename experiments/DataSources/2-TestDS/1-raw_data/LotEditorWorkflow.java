...
import org.imogene.epicam.client.ui.editor.LotEditor;
...
/**
 * Worflow that manages the life of a LotProxy in the UI
 * @author MEDES-IMPS
 */
public class LotEditorWorkflow extends EditorWorkflowComposite {

	interface Driver extends RequestFactoryEditorDriver<LotProxy, LotEditor> {
	}

	private EpicamRequestFactory requestFactory;

	private LotRequest request;
	public LotProxy current;
	private Driver editorDriver;
	private LotEditor editor;
	private String initField;
	private boolean showGlassPanel = false;

	/**
	 * Workflow constructor for the creation of a Lot instance
	 * @param factory the application request factory
	 * @param titleContainer the Label that will display the workflow title
	 */
	public LotEditorWorkflow(EpicamRequestFactory factory, Label titleContainer) {
	 ...
	 */
	public LotEditorWorkflow(EpicamRequestFactory factory,
		...
		if (parent != null) {
			editor = new LotEditor(factory, true);
			this.initField = initField;
		} else
			editor = new LotEditor(factory);

		isNew = true;
		setEditable(true);

		setTitle(NLS.constants().lot_create_title());
		createDriver();
		createNewLot();
	...
	/**
	 * Workflow constructor for the visualization and edition of an existing Lot instance
	 * @param factory the application request factory
	 * @param entityId the id of the Lot instance to be visualized and edited	 
	 * @param titleContainer the Label that will display the workflow title	 
	 */
	public LotEditorWorkflow(EpicamRequestFactory factory, String entityId,
	...
	/**
	 * Create a new instance of Lot
	 */
	private void createNewLot() {

		request = requestFactory.lotRequest();

		/* create a new intance of Lot */
		LotProxy newLot = request.create(LotProxy.class);
		newLot.setId(ImogKeyGenerator.generateKeyId("LOT"));
	...
	/**
	 * Get an existing instance of Lot
	 * @param entityId the id of the LotProxy to be fetched
	 */
	private void fetchLot(String entityId) {

		LotRequest request = requestFactory.lotRequest();

		/* get the Lot instance from database */
		Request<LotProxy> fetchRequest = request.findById(entityId);
		...
		fetchRequest.to(new Receiver<LotProxy>() {
			@Override
			public void onSuccess(LotProxy entity) {
				viewLot(entity);
			}
		}).fire();
	}
	...
	/**
	 * Display the current instance of Lot in editor
	 * @param entity the LotProxy to be displayed
	 */
	private void viewLot(LotProxy entity) {

		/* display instance information */
		setTitle(NLS.constants().lot_name() + ": "
	...
	/**
	 * Edit the current instance of Lot in editor
	 */
	@Override
	protected void edit() {
	...
	/**
	 * Persist the current instance of Lot
	 */
	@Override
	protected void save() {
	 ...
	 * 
	 */
	private void checkLotUnicityForCdt() {

		Request<Boolean> checkLotRequest = requestFactory.lotRequest()
				.isLotUniqueForCdt(editor.getNumeroLot(),
						editor.getCdtLot().getId());
		checkLotRequest.to(new Receiver<Boolean>() {

			@Override
			public void onSuccess(Boolean isUnique) {
				if (isUnique != null && !isUnique)
					editor.raiseLotNotUniqueError();
	...
	private void fireSaveRequest() {
		...
		if (editorDriver.hasErrors()) {
			//Window.alert("Lot form not validated locally");
			return;
		}
		...
		saveRequest.to(new Receiver<Void>() {
			...
			public void onSuccess(Void response) {
				requestFactory.getEventBus().fireEvent(
						new SaveLotEvent(current, initField));
				closeForm();
			}

			@Override
			public void onConstraintViolation(Set<ConstraintViolation<?>> errors) {
				//Window.alert("Lot form not validated on server");
			...
			@Override
			public void onFailure(ServerFailure error) {
				Window.alert("Error updating the Lot");
				super.onFailure(error);
			}
		});
	...
	protected void cancel() {
		if (parent != null)
			...
			if (isNew)
				...
				requestFactory.getEventBus().fireEvent(
						new ViewLotEvent(current.getId(), closeEvent));
		}
	...
	protected void returnToList() {
		requestFactory.getEventBus().fireEvent(new ListLotEvent());
	}
