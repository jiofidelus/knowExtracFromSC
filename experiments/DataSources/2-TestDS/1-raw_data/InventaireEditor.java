...
import org.imogene.epicam.client.ui.workflow.panel.DetailInventaireFormPanel;
...
import org.imogene.epicam.client.ui.editor.nested.InventaireDetailsListEditor;
import org.imogene.epicam.shared.request.InventaireRequest;

/**
 * Editor that provides the UI components that allow a InventaireProxy to be viewed and edited
 * @author MEDES-IMPS
 */
public class InventaireEditor extends Composite
		implements
			Editor<InventaireProxy>,
			HasEditorDelegate<InventaireProxy>,
			HasEditorErrors<InventaireProxy> {

	interface Binder extends UiBinder<Widget, InventaireEditor> {
	}
	...
	private EditorDelegate<InventaireProxy> delegate;
	private CommonFieldUtil commonFieldUtil = CommonFieldUtil.get();

	private InventaireProxy editedValue; //Not used by the editor
	...
	@UiField(provided = true)
	InventaireDetailsListEditor details;
	private DetailInventaireDataProvider detailsDataProvider;
	 ...
	 */
	public InventaireEditor(EpicamRequestFactory factory, boolean hideButtons) {
	 ...
	 * @param factory the application request factory
	 */
	public InventaireEditor(EpicamRequestFactory factory) {
		this(factory, false);
	}
	...
	private void properties() {
		...
		informationsDepartSection.setGroupTitle(NLS.constants()
				.inventaire_group_informationsDepart());
		date.setLabel(NLS.constants().inventaire_field_date());
		region.setLabel(NLS.constants().inventaire_field_region());
		// the value of region affects the value of other fields
		region.notifyChanges(requestFactory.getEventBus());
		districtSante
				.setLabel(NLS.constants().inventaire_field_districtSante());
		// the value of districtSante affects the value of other fields
		districtSante.notifyChanges(requestFactory.getEventBus());
		CDT.setLabel(NLS.constants().inventaire_field_cDT());
	...
	private void setRelationFields() {
		...
		/* field  details */
		details = new InventaireDetailsListEditor(requestFactory);
	...
	private void setFieldEditAccess() {
		...
		/* InformationsDepart section widgets visibility */
		if (!AccessManager.canEditInventaireInformationsDepart())
			informationsDepartSection.setVisible(false);
	 ...
	 */
	public void setRequestContextForListEditors(InventaireRequest ctx) {
	...
	/**
	 * Gets the InventaireProxy that is edited in the Workflow
	 * Not used by the editor
	 * Temporary storage used to transmit the proxy to related entities
	 * @return
	 */
	public InventaireProxy getEditedValue() {
		return editedValue;
	}

	/**
	 * Sets the InventaireProxy that is edited in the Workflow
	 * Not used by the editor
	 * Temporary storage used to transmit the proxy to related entities	 
	 * @param editedValue 
	 */
	public void setEditedValue(InventaireProxy editedValue) {
	...
	@Override
	public void setDelegate(EditorDelegate<InventaireProxy> delegate) {
