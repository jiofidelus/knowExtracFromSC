...
import org.imogene.epicam.client.ui.workflow.panel.InventaireFormPanel;
 ...
 * @author MEDES-IMPS
 */
public class DetailInventaireEditor extends Composite
		implements
			Editor<DetailInventaireProxy>,
			HasEditorDelegate<DetailInventaireProxy>,
			HasEditorErrors<DetailInventaireProxy> {

	interface Binder extends UiBinder<Widget, DetailInventaireEditor> {
	}
	...
	private DetailInventaireProxy editedValue; //Not used by the editor
	...
	ImogSingleRelationBox<InventaireProxy> inventaire;
	private InventaireDataProvider inventaireDataProvider;
	@UiField(provided = true)
	 ...
	 */
	public DetailInventaireEditor(EpicamRequestFactory factory,
			boolean hideButtons) {
	...
	private void properties() {
		...
		descriptionSection.setGroupTitle(NLS.constants()
				.detailInventaire_group_description());
		inventaire
				.setLabel(NLS.constants().detailInventaire_field_inventaire());
		// hidden field
	...
	private void setRelationFields() {
		...
		/* field  inventaire */
		inventaireDataProvider = new InventaireDataProvider(requestFactory);
		if (hideButtons) // in popup, relation buttons hidden
			inventaire = new ImogSingleRelationBox<InventaireProxy>(
					inventaireDataProvider, EpicamRenderer.get(), true);
		else {// in wrapper panel, relation buttons enabled												
			if (AccessManager.canCreateInventaire()
					&& AccessManager.canEditInventaire())
				inventaire = new ImogSingleRelationBox<InventaireProxy>(
						inventaireDataProvider, EpicamRenderer.get());
			else
				inventaire = new ImogSingleRelationBox<InventaireProxy>(false,
						inventaireDataProvider, EpicamRenderer.get());
		}
	 ...
	 */
	public void setRequestContextForListEditors(DetailInventaireRequest ctx) {
	}
	...
	public void setInventaire(InventaireProxy value, boolean isLocked) {
		inventaire.setLocked(isLocked);
		inventaire.setValue(value);

	}

	/** Widget update for field inventaire */
	private void clearInventaireWidget() {
		inventaire.clear();
	}
	...
	private void setRelationHandlers() {
		...
		/* 'Information' button for field Inventaire */
		inventaire.setViewClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				if (inventaire.getValue() != null) {
					RelationPopupPanel relationPopup = new RelationPopupPanel();
					InventaireFormPanel form = new InventaireFormPanel(
							requestFactory, inventaire.getValue().getId(),
							relationPopup, "inventaire");
		...
		/* 'Add' button for field Inventaire */
		inventaire.setAddClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {
				RelationPopupPanel relationPopup = new RelationPopupPanel();
				InventaireFormPanel form = new InventaireFormPanel(
						requestFactory, null, relationPopup, "inventaire");
				/* common fields */
		...
		/* SaveEvent handler when a Inventaire is created or updated from the relation field Inventaire */
		registrations.add(requestFactory.getEventBus().addHandler(
				SaveInventaireEvent.TYPE, new SaveInventaireEvent.Handler() {
					@Override
					public void saveInventaire(InventaireProxy value) {
						inventaire.setValue(value);
					}
					@Override
					public void saveInventaire(InventaireProxy value,
							String initField) {
						if (initField.equals("inventaire"))
							inventaire.setValue(value, true);
					}
				}));
	...
	/**
	 * Gets the DetailInventaireProxy that is edited in the Workflow
	 * Not used by the editor
	 * Temporary storage used to transmit the proxy to related entities
	 * @return
	 */
	public DetailInventaireProxy getEditedValue() {
		return editedValue;
	}

	/**
	 * Sets the DetailInventaireProxy that is edited in the Workflow
	 * Not used by the editor
	 * Temporary storage used to transmit the proxy to related entities	 
	 * @param editedValue 
	 */
	public void setEditedValue(DetailInventaireProxy editedValue) {
	...
	private void setAllLabelWith(String width) {
		...
		inventaire.setLabelWidth(width);
		CDT.setLabelWidth(width);
		lot.setLabelWidth(width);
		quantiteReelle.setLabelWidth(width);
		quantiteTheorique.setLabelWidth(width);
