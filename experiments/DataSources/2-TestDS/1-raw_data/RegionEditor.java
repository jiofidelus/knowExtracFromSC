...
/**
 * Editor that provides the UI components that allow a RegionProxy to be viewed and edited
 * @author MEDES-IMPS
 */
public class RegionEditor extends Composite
		implements
			Editor<RegionProxy>,
			HasEditorDelegate<RegionProxy>,
			HasEditorErrors<RegionProxy> {
	...
	private EditorDelegate<RegionProxy> delegate;

	private RegionProxy editedValue; //Not used by the editor
	...
	@UiField(provided = true)
	ImogMultiRelationBox<DistrictSanteProxy> districtSantes;
	private DistrictSanteDataProvider districtSantesDataProvider;
	 ...
	 */
	public RegionEditor(EpicamRequestFactory factory, boolean hideButtons) {
	 ...
	 * @param factory the application request factory
	 */
	public RegionEditor(EpicamRequestFactory factory) {
		this(factory, false);
	}
	...
	private void properties() {
		...
		descriptionSection.setGroupTitle(NLS.constants()
				.region_group_description());
		code.setLabel(NLS.constants().region_field_code());
		nom.setLabel(NLS.constants().region_field_nom());
		description.setLabel(NLS.constants().region_field_description());
		districtSantes.setLabel(NLS.constants().region_field_districtSantes());

		/* Adresse section widgets */
		adresseSection.setGroupTitle(NLS.constants().region_group_adresse());
		libelle.setLabel(NLS.constants().region_field_libelle());
		libelle.addItem(EpicamEnumConstants.REGION_LIBELLE_DOMICILE, NLS
				.constants().region_libelle_domicile_option());
		libelle.addItem(EpicamEnumConstants.REGION_LIBELLE_BUREAU, NLS
				.constants().region_libelle_bureau_option());
		libelle.addItem(EpicamEnumConstants.REGION_LIBELLE_AUTRE, NLS
				.constants().region_libelle_autre_option());
		complementAdresse.setLabel(NLS.constants()
				.region_field_complementAdresse());
		quartier.setLabel(NLS.constants().region_field_quartier());
		ville.setLabel(NLS.constants().region_field_ville());
		codePostal.setLabel(NLS.constants().region_field_codePostal());
		coordonnees.setLabel(NLS.constants().region_field_coordonnees());
	...
	private void setRelationFields() {
		...
		/* field  districtSantes */
		districtSantesDataProvider = new DistrictSanteDataProvider(
				requestFactory, "region");
		if (hideButtons) // in popup, relation buttons hidden		
			districtSantes = new ImogMultiRelationBox<DistrictSanteProxy>(
					districtSantesDataProvider, EpicamRenderer.get(), true);
		else {// in wrapper panel, relation buttons enabled
			if (AccessManager.canCreateDistrictSante()
					&& AccessManager.canEditDistrictSante())
				districtSantes = new ImogMultiRelationBox<DistrictSanteProxy>(
						districtSantesDataProvider, EpicamRenderer.get(), null);
			else
				districtSantes = new ImogMultiRelationBox<DistrictSanteProxy>(
						false, districtSantesDataProvider,
						EpicamRenderer.get(), null);
		}
		districtSantes.setPopUpTitle(NLS.constants()
				.districtSante_select_title());
		districtSantes.setFilterPanel(new DistrictSanteFilterPanel());
	...
	public void setEdited(boolean isEdited) {
		...
		districtSantes.setEdited(isEdited);
	...
	private void setFieldReadAccess() {
		...
		if (!AccessManager.canReadRegionAdresse())
	...
	private void setFieldEditAccess() {
		...
		/* Description section widgets visibility */
		if (!AccessManager.canEditRegionDescription())
			descriptionSection.setVisible(false);

		/* Adresse section widgets visibility */
		if (!AccessManager.canEditRegionAdresse())
			adresseSection.setVisible(false);
	 ...
	 */
	public void setRequestContextForListEditors(RegionRequest ctx) {
	}
	...
	/** Widget update for field districtSantes */
	private void clearDistrictSantesWidget() {
		districtSantes.emptyList();
	}
	...
	private void setRelationHandlers() {
		...
		/* 'Information' button for field DistrictSantes */
		districtSantes.setViewClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {

				if (!districtSantes.isEmpty()
						&& districtSantes.getSelectedEntities().size() > 0) {

					DistrictSanteProxy value = districtSantes
		...
		/* 'Add' button for field DistrictSantes */
		districtSantes.setAddClickHandler(new ClickHandler() {
			@Override
			public void onClick(ClickEvent event) {

				RelationPopupPanel relationPopup = new RelationPopupPanel();
				DistrictSanteFormPanel form = new DistrictSanteFormPanel(
						requestFactory, null, relationPopup, "districtSantes");
				form.setRegion(editedValue, true);
				/* common fields */
		...
		/* SaveEvent handler when a DistrictSante is created or updated from the relation field DistrictSantes */
		registrations.add(requestFactory.getEventBus().addHandler(
				SaveDistrictSanteEvent.TYPE,
				new SaveDistrictSanteEvent.Handler() {
					@Override
					public void saveDistrictSante(DistrictSanteProxy value) {
						if (!districtSantes.isPresent(value))
							districtSantes.addValue(value);
					}
					public void saveDistrictSante(DistrictSanteProxy value,
							String initField) {
						if (initField.equals("districtSantes")) {
							if (districtSantes.isPresent(value))
								districtSantes.replaceValue(value);
							else
								districtSantes.addValue(value, true);
						}
					}
				}));
	...
	/**
	 * Gets the RegionProxy that is edited in the Workflow
	 * Not used by the editor
	 * Temporary storage used to transmit the proxy to related entities
	 * @return
	 */
	public RegionProxy getEditedValue() {
		return editedValue;
	}

	/**
	 * Sets the RegionProxy that is edited in the Workflow
	 * Not used by the editor
	 * Temporary storage used to transmit the proxy to related entities	 
	 * @param editedValue 
	 */
	public void setEditedValue(RegionProxy editedValue) {
	...
	private void setAllLabelWith(String width) {
		...
		districtSantes.setLabelWidth(width);
	...
	private void setAllBoxWith(int width) {
		...
		districtSantes.setBoxWidth(width);
	...
	@Override
	public void setDelegate(EditorDelegate<RegionProxy> delegate) {
