...
import org.imogene.epicam.client.ui.editor.RegionEditor;
...
/**
 * Worflow that manages the life of a RegionProxy in the UI
 * @author MEDES-IMPS
 */
public class RegionEditorWorkflow extends EditorWorkflowComposite {

	interface Driver
			extends
				RequestFactoryEditorDriver<RegionProxy, RegionEditor> {
	}

	private EpicamRequestFactory requestFactory;

	private RegionRequest request;
	public RegionProxy current;
	private Driver editorDriver;
	private RegionEditor editor;
	private String initField;
	private boolean showGlassPanel = false;

	/**
	 * Workflow constructor for the creation of a Region instance
	 * @param factory the application request factory
	 * @param titleContainer the Label that will display the workflow title
	 */
	public RegionEditorWorkflow(EpicamRequestFactory factory,
	...
	public RegionEditorWorkflow(EpicamRequestFactory factory,
		...
		if (parent != null) {
			editor = new RegionEditor(factory, true);
			this.initField = initField;
		} else
			editor = new RegionEditor(factory);

		isNew = true;
		setEditable(true);

		setTitle(NLS.constants().region_create_title());
		createDriver();
		createNewRegion();
	...
	/**
	 * Workflow constructor for the visualization and edition of an existing Region instance
	 * @param factory the application request factory
	 * @param entityId the id of the Region instance to be visualized and edited	 
	 * @param titleContainer the Label that will display the workflow title	 
	 */
	public RegionEditorWorkflow(EpicamRequestFactory factory, String entityId,
			Label titleContainer) {
	...
	private void createNewRegion() {
		...
		request = requestFactory.regionRequest();

		/* create a new intance of Region */
		RegionProxy newRegion = request.create(RegionProxy.class);
		newRegion.setId(ImogKeyGenerator.generateKeyId("REG"));
		LocalizedTextProxy newNom = request.create(LocalizedTextProxy.class);
		newRegion.setNom(newNom);
		LocalizedTextProxy newDescription = request
				.create(LocalizedTextProxy.class);
		newRegion.setDescription(newDescription);
		GeoFieldProxy newCoordonnees = request.create(GeoFieldProxy.class);
		newRegion.setCoordonnees(newCoordonnees);
	...
	/**
	 * Get an existing instance of Region
	 * @param entityId the id of the RegionProxy to be fetched
	 */
	private void fetchRegion(String entityId) {

		RegionRequest request = requestFactory.regionRequest();

		/* get the Region instance from database */
		Request<RegionProxy> fetchRequest = request.findById(entityId);
		fetchRequest.with("nom");
		fetchRequest.with("description");
		fetchRequest.with("districtSantes");
		fetchRequest.with("districtSantes.nom");
		fetchRequest.with("coordonnees");

		fetchRequest.to(new Receiver<RegionProxy>() {
			@Override
			public void onSuccess(RegionProxy entity) {
				viewRegion(entity);
			}
		}).fire();
	}
	...
	/**
	 * Display the current instance of Region in editor
	 * @param entity the RegionProxy to be displayed
	 */
	private void viewRegion(RegionProxy entity) {

		/* display instance information */
		setTitle(NLS.constants().region_name() + ": "
	...
	/**
	 * Edit the current instance of Region in editor
	 */
	@Override
	protected void edit() {
	...
	/**
	 * Persist the current instance of Region
	 */
	@Override
	protected void save() {
		...
		if (editorDriver.hasErrors()) {
			//Window.alert("Region form not validated locally");
			return;
		}
		...
		saveRequest.to(new Receiver<Void>() {
			...
			public void onSuccess(Void response) {
				requestFactory.getEventBus().fireEvent(
						new SaveRegionEvent(current, initField));
				closeForm();
			}

			@Override
			public void onConstraintViolation(Set<ConstraintViolation<?>> errors) {
				//Window.alert("Region form not validated on server");
			...
			public void onFailure(ServerFailure error) {
				Window.alert("Error updating the Region");
	...
	protected void cancel() {
		if (parent != null)
			...
			if (isNew)
				...
				requestFactory.getEventBus().fireEvent(
						new ViewRegionEvent(current.getId(), closeEvent));
		}
	...
	protected void returnToList() {
		requestFactory.getEventBus().fireEvent(new ListRegionEvent());
	}
