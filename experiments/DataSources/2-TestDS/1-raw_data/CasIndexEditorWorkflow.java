...
import org.imogene.epicam.client.ui.editor.CasIndexEditor;
...
/**
 * Worflow that manages the life of a CasIndexProxy in the UI
 * @author MEDES-IMPS
 */
public class CasIndexEditorWorkflow extends EditorWorkflowComposite {

	interface Driver
			extends
				RequestFactoryEditorDriver<CasIndexProxy, CasIndexEditor> {
	}

	private EpicamRequestFactory requestFactory;

	private CasIndexRequest request;
	public CasIndexProxy current;
	private Driver editorDriver;
	private CasIndexEditor editor;
	private String initField;
	private boolean showGlassPanel = false;

	/**
	 * Workflow constructor for the creation of a CasIndex instance
	 * @param factory the application request factory
	 * @param titleContainer the Label that will display the workflow title
	 */
	public CasIndexEditorWorkflow(EpicamRequestFactory factory,
	...
	public CasIndexEditorWorkflow(EpicamRequestFactory factory,
		...
		if (parent != null) {
			editor = new CasIndexEditor(factory, true);
			this.initField = initField;
		} else
			editor = new CasIndexEditor(factory);

		isNew = true;
		setEditable(true);

		setTitle(NLS.constants().casIndex_create_title());
		createDriver();
		createNewCasIndex();
	...
	/**
	 * Workflow constructor for the visualization and edition of an existing CasIndex instance
	 * @param factory the application request factory
	 * @param entityId the id of the CasIndex instance to be visualized and edited	 
	 * @param titleContainer the Label that will display the workflow title	 
	 */
	public CasIndexEditorWorkflow(EpicamRequestFactory factory,
	...
	/**
	 * Create a new instance of CasIndex
	 */
	private void createNewCasIndex() {

		request = requestFactory.casIndexRequest();

		/* create a new intance of CasIndex */
	...
	/**
	 * Get an existing instance of CasIndex
	 * @param entityId the id of the CasIndexProxy to be fetched
	 */
	private void fetchCasIndex(String entityId) {

		CasIndexRequest request = requestFactory.casIndexRequest();

		/* get the CasIndex instance from database */
		Request<CasIndexProxy> fetchRequest = request.findById(entityId);
		fetchRequest.with("patient");
		fetchRequest.with("patientLie");

		fetchRequest.to(new Receiver<CasIndexProxy>() {
			@Override
			public void onSuccess(CasIndexProxy entity) {
				viewCasIndex(entity);
			}
		}).fire();
	}
	...
	/**
	 * Display the current instance of CasIndex in editor
	 * @param entity the CasIndexProxy to be displayed
	 */
	private void viewCasIndex(CasIndexProxy entity) {

		/* display instance information */
		setTitle(NLS.constants().casIndex_name() + ": "
	...
	/**
	 * Edit the current instance of CasIndex in editor
	 */
	@Override
	protected void edit() {
	...
	/**
	 * Persist the current instance of CasIndex
	 */
	@Override
	protected void save() {
		...
		if (editorDriver.hasErrors()) {
			//Window.alert("CasIndex form not validated locally");
			return;
		}
		...
		saveRequest.to(new Receiver<Void>() {
			...
			public void onSuccess(Void response) {
				requestFactory.getEventBus().fireEvent(
						new SaveCasIndexEvent(current, initField));
				closeForm();
			}

			@Override
			public void onConstraintViolation(Set<ConstraintViolation<?>> errors) {
				//Window.alert("CasIndex form not validated on server");
			...
			public void onFailure(ServerFailure error) {
				Window.alert("Error updating the CasIndex");
	...
	protected void cancel() {
		if (parent != null)
			...
			if (isNew)
				...
				requestFactory.getEventBus().fireEvent(
						new ViewCasIndexEvent(current.getId(), closeEvent));
		}
	...
	protected void returnToList() {
		requestFactory.getEventBus().fireEvent(new ListCasIndexEvent());
	}
	...
	public void setPatientLie(PatientProxy value, boolean isLocked) {
		editor.setPatientLie(value, isLocked);
	}
