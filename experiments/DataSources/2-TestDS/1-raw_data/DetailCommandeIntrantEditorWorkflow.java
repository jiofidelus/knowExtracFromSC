...
import org.imogene.epicam.client.ui.editor.DetailCommandeIntrantEditor;
...
/**
 * Worflow that manages the life of a DetailCommandeIntrantProxy in the UI
 * @author MEDES-IMPS
 */
public class DetailCommandeIntrantEditorWorkflow
		extends
			EditorWorkflowComposite {

	interface Driver
			extends
				RequestFactoryEditorDriver<DetailCommandeIntrantProxy, DetailCommandeIntrantEditor> {
	}

	private EpicamRequestFactory requestFactory;

	private DetailCommandeIntrantRequest request;
	public DetailCommandeIntrantProxy current;
	private Driver editorDriver;
	private DetailCommandeIntrantEditor editor;
	private String initField;
	private boolean showGlassPanel = false;

	/**
	 * Workflow constructor for the creation of a DetailCommandeIntrant instance
	 * @param factory the application request factory
	 * @param titleContainer the Label that will display the workflow title
	 */
	public DetailCommandeIntrantEditorWorkflow(EpicamRequestFactory factory,
	...
	public DetailCommandeIntrantEditorWorkflow(EpicamRequestFactory factory,
		...
		if (parent != null) {
			editor = new DetailCommandeIntrantEditor(factory, true);
			this.initField = initField;
		} else
			editor = new DetailCommandeIntrantEditor(factory);

		isNew = true;
		setEditable(true);

		setTitle(NLS.constants().detailCommandeIntrant_create_title());
		createDriver();
		createNewDetailCommandeIntrant();

		this.setContent(editor);
	}

	/**
	 * Workflow constructor for the visualization and edition of an existing DetailCommandeIntrant instance
	 * @param factory the application request factory
	 * @param entityId the id of the DetailCommandeIntrant instance to be visualized and edited	 
	 * @param titleContainer the Label that will display the workflow title	 
	 */
	public DetailCommandeIntrantEditorWorkflow(EpicamRequestFactory factory,
	...
	public DetailCommandeIntrantEditorWorkflow(EpicamRequestFactory factory,
		...
		createDriver();
		fetchDetailCommandeIntrant(entityId);
	...
	/**
	 * Create a new instance of DetailCommandeIntrant
	 */
	private void createNewDetailCommandeIntrant() {

		request = requestFactory.detailCommandeIntrantRequest();

		/* create a new intance of DetailCommandeIntrant */
		...
		current = newDetailCommandeIntrant;
	...
	/**
	 * Get an existing instance of DetailCommandeIntrant
	 * @param entityId the id of the DetailCommandeIntrantProxy to be fetched
	 */
	private void fetchDetailCommandeIntrant(String entityId) {

		DetailCommandeIntrantRequest request = requestFactory
				.detailCommandeIntrantRequest();

		/* get the DetailCommandeIntrant instance from database */
		Request<DetailCommandeIntrantProxy> fetchRequest = request
				.findById(entityId);
		fetchRequest.with("commande");
		fetchRequest.with("commande.CDT");
		fetchRequest.with("intrant");

		fetchRequest.to(new Receiver<DetailCommandeIntrantProxy>() {
			@Override
			public void onSuccess(DetailCommandeIntrantProxy entity) {
				viewDetailCommandeIntrant(entity);
			}
		}).fire();
	}

	/**
	 * Display the current instance of DetailCommandeIntrant in editor
	 * @param entity the DetailCommandeIntrantProxy to be displayed
	 */
	private void viewDetailCommandeIntrant(DetailCommandeIntrantProxy entity) {

		/* display instance information */
		setTitle(NLS.constants().detailCommandeIntrant_name() + ": "
	...
	/**
	 * Edit the current instance of DetailCommandeIntrant in editor
	 */
	@Override
	protected void edit() {
	...
	/**
	 * Persist the current instance of DetailCommandeIntrant
	 */
	@Override
	protected void save() {
		...
		if (editorDriver.hasErrors()) {
			//Window.alert("DetailCommandeIntrant form not validated locally");
			return;
		}
		...
		saveRequest.to(new Receiver<Void>() {
			...
			public void onSuccess(Void response) {
				requestFactory.getEventBus().fireEvent(
						new SaveDetailCommandeIntrantEvent(current, initField));
				closeForm();
			}

			@Override
			public void onConstraintViolation(Set<ConstraintViolation<?>> errors) {
				//Window.alert("DetailCommandeIntrant form not validated on server");
			...
			public void onFailure(ServerFailure error) {
				Window.alert("Error updating the DetailCommandeIntrant");
	...
	protected void cancel() {
		if (parent != null)
			...
			if (isNew)
				...
				requestFactory.getEventBus().fireEvent(
						new ViewDetailCommandeIntrantEvent(current.getId(),
								closeEvent));
		}
