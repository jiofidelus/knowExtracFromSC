...
import org.imogene.epicam.client.event.create.CreateExamenATBEvent;
...
/**
 * Composite that displays the list of ExamenATB entries
 * @author MEDES-IMPS
 */
public class ExamenATBDynaTable extends ImogDynaTable<ExamenATBProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public ExamenATBDynaTable(EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<ExamenATBProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new ExamenATBFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadExamenATBExamen()) {
			Column<ExamenATBProxy, String> casTbColumn = new CasTbColumn();
			casTbColumn.setSortable(true);
			table.addColumn(casTbColumn, NLS.constants()
					.examenATB_field_s_casTb());
		}
		if (AccessManager.canReadExamenATBExamen()) {
			Column<ExamenATBProxy, String> dateExamenColumn = new DateExamenColumn();
			dateExamenColumn.setSortable(true);
			table.addColumn(dateExamenColumn, NLS.constants()
					.examenATB_field_s_dateExamen());
		}
		if (AccessManager.canReadExamenATBExamen()) {
			Column<ExamenATBProxy, String> raisonDepistageColumn = new RaisonDepistageColumn();
			raisonDepistageColumn.setSortable(true);
			table.addColumn(raisonDepistageColumn, NLS.constants()
					.examenATB_field_s_raisonDepistage());
		}
		if (AccessManager.canReadExamenATBExamen()) {
			Column<ExamenATBProxy, String> resultatColumn = new ResultatColumn();
			resultatColumn.setSortable(true);
			table.addColumn(resultatColumn, NLS.constants()
					.examenATB_field_s_resultat());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(ExamenATBProxy value) {
		History.newItem(TokenHelper.TK_VIEW + "/examenatb/" + value.getId(),
				true);
		return null;
	}
		...
		if (AccessManager.canDeleteExamenATB()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<ExamenATBProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						msg.append(BaseNLS.constants()
								...
								+ " "
								+ NLS.constants().examenATB_name()
								+ " "
								+ BaseNLS.constants()
										.confirmation_delete_several2() + ": ");
						int i = 0;
						for (ExamenATBProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getExamenATBRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected ExamenATB entries have been deleted");
									requestFactory.getEventBus().fireEvent(
											new ListExamenATBEvent());
								}

								@Override
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the ExamenATB entries");
	...
	/**
	 * Creates the action command that enables to export the ExamenATB
	 * entries in a csv file
	 * @return the command
	 */
	public Command getCsvExportButton() {

		if (AccessManager.canExportExamenATB()) {

			Command command = new Command() {
				public void execute() {

					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.EXAM_ATB_CSV_KEY + "?"
	...
	private ExamenATBRequest getExamenATBRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.examenATBRequest();
	}
	 ...
	 * @author MEDES-IMPS
	 */
	private class CasTbColumn extends ImogColumn<ExamenATBProxy, String> {
		...
		@Override
		public String getValue(ExamenATBProxy object) {
			String value = null;
			if (object != null) {
	...
	/**
	 * Column for field DateExamen
	 * @author MEDES-IMPS
	 */
	private class DateExamenColumn extends ImogColumn<ExamenATBProxy, String> {

		public DateExamenColumn() {
			super(new TextCell());
		}
		...
		public String getValue(ExamenATBProxy object) {
			String value = null;
			if (object != null) {
				if (object.getDateExamen() == null)
					value = "";
				else
					value = DateUtil.getFormatedDate(object.getDateExamen());
			}
			return value;
		}
	...
	private class RaisonDepistageColumn
			extends
				ImogColumn<ExamenATBProxy, String> {
		...
		public String getValue(ExamenATBProxy object) {
			...
			if (object != null) {
				if (object.getRaisonDepistage() == null)
					...
					value = renderer.getEnumDisplayValue(ExamenATBProxy.class,
							"raisonDepistage", object.getRaisonDepistage());
			}
			return value;
		}
	 ...
	 * @author MEDES-IMPS
	 */
	private class ResultatColumn extends ImogColumn<ExamenATBProxy, String> {
