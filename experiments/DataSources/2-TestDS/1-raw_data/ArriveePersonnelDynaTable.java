...
import org.imogene.epicam.client.event.create.CreateArriveePersonnelEvent;
...
import org.imogene.epicam.client.ui.filter.ArriveePersonnelFilterPanel;
...
/**
 * Composite that displays the list of ArriveePersonnel entries
 * @author MEDES-IMPS
 */
public class ArriveePersonnelDynaTable
		extends
			ImogDynaTable<ArriveePersonnelProxy> {

	private List<HandlerRegistration> registrations = new ArrayList<HandlerRegistration>();

	private PushButton deleteButton;

	public ArriveePersonnelDynaTable(EpicamRequestFactory requestFactory,
			ImogBeanDataProvider<ArriveePersonnelProxy> provider,
			boolean checkBoxesVisible) {
		super(requestFactory, provider, checkBoxesVisible);
	}

	public ImogFilterPanel getFilterPanel() {
		ImogFilterPanel filterPanel = new ArriveePersonnelFilterPanel();
	...
	protected void setColumns() {
		...
		if (AccessManager.canReadArriveePersonnelDescription()) {
			Column<ArriveePersonnelProxy, String> dateArriveeColumn = new DateArriveeColumn();
			dateArriveeColumn.setSortable(true);
			table.addColumn(dateArriveeColumn, NLS.constants()
					.arriveePersonnel_field_s_dateArrivee());
		}
		if (AccessManager.canReadArriveePersonnelDescription()) {
			Column<ArriveePersonnelProxy, String> regionColumn = new RegionColumn();
			regionColumn.setSortable(true);
			table.addColumn(regionColumn, NLS.constants()
					.arriveePersonnel_field_s_region());
		}
		if (AccessManager.canReadArriveePersonnelDescription()) {
			Column<ArriveePersonnelProxy, String> districtSanteColumn = new DistrictSanteColumn();
			districtSanteColumn.setSortable(true);
			table.addColumn(districtSanteColumn, NLS.constants()
					.arriveePersonnel_field_s_districtSante());
		}
		if (AccessManager.canReadArriveePersonnelDescription()) {
			Column<ArriveePersonnelProxy, String> cDTColumn = new CDTColumn();
			cDTColumn.setSortable(true);
			table.addColumn(cDTColumn, NLS.constants()
					.arriveePersonnel_field_s_cDT());
		}
		if (AccessManager.canReadArriveePersonnelDescription()) {
			Column<ArriveePersonnelProxy, String> personnelColumn = new PersonnelColumn();
			personnelColumn.setSortable(true);
			table.addColumn(personnelColumn, NLS.constants()
					.arriveePersonnel_field_s_personnel());
		}

	}

	@Override
	protected GwtEvent<?> getViewEvent(ArriveePersonnelProxy value) {
		History.newItem(
				TokenHelper.TK_VIEW + "/arriveepersonnel/" + value.getId(),
				true);
		return null;
	}
		...
		if (AccessManager.canDeleteArriveePersonnel()
			...
			registrations.add(deleteButton.addClickHandler(new ClickHandler() {
				...
				public void onClick(ClickEvent event) {
					...
					Set<ArriveePersonnelProxy> selectedEntities = selectionModel
							.getSelectedSet();
					...
					if (count > 0) {
						...
						msg.append(BaseNLS.constants()
								...
								+ " "
								+ NLS.constants().arriveePersonnel_name()
								+ " "
								+ BaseNLS.constants()
										.confirmation_delete_several2() + ": ");
						int i = 0;
						for (ArriveePersonnelProxy entity : selectedEntities) {
						...
						if (toDelete) {
							...
							Request<Void> deleteRequest = getArriveePersonnelRequest()
									.delete(selectedEntities);
							deleteRequest.fire(new Receiver<Void>() {
								@Override
								public void onSuccess(Void response) {
									//Window.alert("The selected ArriveePersonnel entries have been deleted");
									requestFactory.getEventBus().fireEvent(
											new ListArriveePersonnelEvent());
								}

								@Override
								public void onFailure(ServerFailure error) {
									Window.alert("Error deleting the ArriveePersonnel entries");
	...
	/**
	 * Creates the action command that enables to export the ArriveePersonnel
	 * entries in a csv file
	 * @return the command
	 */
	public Command getCsvExportButton() {

		if (AccessManager.canExportArriveePersonnel()) {

			Command command = new Command() {
				public void execute() {

					String url = GWT.getHostPageBaseURL()
							+ EpicamBirtConstants.ARR_PERS_CSV_KEY + "?"
							+ EpicamBirtConstants.REPORT_NAME
							+ "=arriveePersonnel_csv" + "&"
	...
	private ArriveePersonnelRequest getArriveePersonnelRequest() {
		EpicamRequestFactory epicamRequestFactory = (EpicamRequestFactory) requestFactory;
		return epicamRequestFactory.arriveePersonnelRequest();
	}
	...
	private class DateArriveeColumn
			extends
				ImogColumn<ArriveePersonnelProxy, String> {
		...
		@Override
		public String getValue(ArriveePersonnelProxy object) {
			String value = null;
			if (object != null) {
	 ...
	 * @author MEDES-IMPS
	 */
	private class CDTColumn extends ImogColumn<ArriveePersonnelProxy, String> {
